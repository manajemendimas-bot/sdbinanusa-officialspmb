...
import { Search, Edit, CheckCircle2, XCircle, Clock3, Lock, Unlock, X, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl border border-[#E2E8F0] shadow-2xs flex flex-col md:flex-row gap-3 items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-xs text-[#64748B] w-full md:w-auto"><span>Filter Status:</span><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as AnnouncementStatus | 'All')} className="py-1.5 px-2.5 rounded-md border border-[#CBD5E1] text-xs bg-[#F5F8FC] focus:outline-hidden"><option value="All">Semua Status ({registrants.length})</option><option value="Diterima">Diterima</option><option value="Lulus Bersyarat (Tes Ulang)">Lulus Bersyarat (Tes Ulang)</option><option value="Belum Diterima">Belum Diterima</option><option value="Dalam Proses">Dalam Proses</option></select></div>
      </div>
              ...
              {filteredRegistrants.length === 0 ? <tr><td colSpan={7} className="py-8 text-center text-xs text-[#64748B]">Tidak ada data pengumuman yang sesuai filter.</td></tr> : filteredRegistrants.map((r) => {
                ...
                return (
                  <tr key={r.id} className="hover:bg-[#F5F8FC] transition-colors">
                    ...
                    <td className="py-3 px-4"><span className={`px-2.5 py-1 rounded-md text-[11px] font-bold inline-flex items-center gap-1.5 ${status === 'Diterima' ? 'bg-emerald-50 text-emerald-800 border border-emerald-300' : status === 'Lulus Bersyarat (Tes Ulang)' ? 'bg-indigo-50 text-indigo-800 border border-indigo-300' : status === 'Belum Diterima' ? 'bg-rose-50 text-rose-800 border border-rose-300' : 'bg-amber-50 text-amber-800 border border-amber-300'}`}>{status === 'Diterima' && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />}{status === 'Lulus Bersyarat (Tes Ulang)' && <AlertTriangle className="w-3.5 h-3.5 text-indigo-600" />}{status === 'Belum Diterima' && <XCircle className="w-3.5 h-3.5 text-rose-600" />}{status === 'Dalam Proses' && <Clock3 className="w-3.5 h-3.5 text-amber-600" />}<span>{status}</span></span></td>
                    ...
                    <td className="py-3 px-4 text-right"><div className="flex items-center justify-end gap-1"><button onClick={() => handleQuickStatus(r.id, 'Diterima')} className="px-2 py-1 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 rounded text-[10px] font-bold cursor-pointer">Lulus</button><button onClick={() => handleQuickStatus(r.id, 'Lulus Bersyarat (Tes Ulang)')} className="px-2 py-1 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded text-[10px] font-bold cursor-pointer">Bersyarat</button><button onClick={() => handleQuickStatus(r.id, 'Belum Diterima')} className="px-2 py-1 bg-rose-50 text-rose-700 hover:bg-rose-100 rounded text-[10px] font-bold cursor-pointer">Tidak</button><button onClick={() => handleOpenEdit(r)} className="p-1 text-[#334155] hover:bg-[#03357E]/10 hover:text-[#03357E] rounded cursor-pointer"><Edit className="w-3.5 h-3.5" /></button></div></td>
                  </tr>
                );
              })}
            </tbody>
